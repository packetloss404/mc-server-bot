...
export class CommandCenter {
  ...
  async dispatchCommand(command: CommandRecord): Promise<CommandRecord> {
    ...
    // Auto-override: if a manual command was dispatched to a bot with a non-manual role,
    // set an override so the role system knows the user has taken manual control.
    this.applyAutoOverride(command);

    return command;
  }

  /** Set override on target bots when a manual command overrides an autonomous/assisted role. */
  private applyAutoOverride(command: CommandRecord): void {
    if (!this.roleManager) return;
    const manualSources: CommandSource[] = ['dashboard', 'hotkey'];
    if (!manualSources.includes(command.source)) return;
    if (command.status !== 'succeeded') return;

    for (const botName of command.targets) {
      const assignment = this.roleManager.getAssignmentForBot(botName);
      if (assignment && assignment.autonomyLevel !== 'manual') {
        this.roleManager.setOverride(botName, 'Manual command', command.id);
      }
    }
  }
