...
// ── Suggested commands for empty/vague input ─────────────
...
export class CommanderService {
  ...
  getSuggestedCommands(): string[] {
    // Return a shuffled subset of suggestions
    const shuffled = [...SUGGESTED_COMMANDS].sort(() => Math.random() - 0.5);
  ...
  private generateClarificationQuestions(
    ...
    for (const { pattern, question, options, field } of ambiguousPatterns) {
      if (pattern.test(input) && confidence < 0.7) {
        ...
        break; // Only one action clarification at a time
      }
    }
  ...
  async parse(input: string): Promise<CommanderPlan> {
    ...
    // Simulate parsing -- in production this calls the LLM
    // For now, derive intent and confidence heuristically
    let intent = '';
    let confidence = 0;
    ...
    // Check for specificity -- boost or lower confidence
    ...
    // If still lacking specifics, flag it
  ...
  async parseWithClarification(
    ...
    // Build an augmented input that includes clarification answers
