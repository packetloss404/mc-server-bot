...
export interface RoleMetrics {
  ...
  autonomyLevelDistribution: Record<string, number>;
  overrideCount: number;
  overrideAvgDurationMs: number | null;
  approvalCountsByStatus: Record<string, number>;
  automationGenerationCount: number;
}
...
export class RoleManager {
  ...
  private automationGenerationCount = 0;
  ...
  getMetrics(): RoleMetrics {
    ...
    const autonomyLevelDistribution: Record<string, number> = {};
    for (const level of VALID_AUTONOMY) {
      autonomyLevelDistribution[level] = 0;
    }
    for (const a of this.assignments) {
      autonomyLevelDistribution[a.autonomyLevel] = (autonomyLevelDistribution[a.autonomyLevel] ?? 0) + 1;
    }

    // Override count and average duration
    const now = Date.now();
    const overrideCount = this.overrides.size;
    let overrideAvgDurationMs: number | null = null;
    if (overrideCount > 0) {
      let totalDuration = 0;
      for (const record of this.overrides.values()) {
        totalDuration += now - record.at;
      }
      overrideAvgDurationMs = Math.round(totalDuration / overrideCount);
    }
    ...
    for (const req of this.approvalRequests) {
      approvalCountsByStatus[req.status] = (approvalCountsByStatus[req.status] ?? 0) + 1;
    }

    return {
      totalAssignments: this.assignments.length,
      assignmentsByRole,
      autonomyLevelDistribution,
      overrideCount,
      overrideAvgDurationMs,
      approvalCountsByStatus,
      automationGenerationCount: this.automationGenerationCount,
    };
  }
  ...
  evaluateAutomation(botName?: string): void {
    ...
    for (const assignment of candidates) {
      ...
      if (assignment.autonomyLevel === 'assisted') {
        ...
        this.automationGenerationCount++;
