...
export interface SquadMetrics {
  ...
  squadsWithActiveMissions: number;
}
...
export class SquadManager {
  ...
  getMetrics(): SquadMetrics {
    const squads = Array.from(this.squads.values());
    const totalSquads = squads.length;

    let totalMembers = 0;
    let squadsWithActiveMissions = 0;

    for (const squad of squads) {
      totalMembers += squad.botNames.length;
      if (squad.activeMissionId) {
        squadsWithActiveMissions++;
      }
    }

    const averageSquadSize = totalSquads > 0
      ? Math.round((totalMembers / totalSquads) * 100) / 100
      : 0;
