...
export function createAPIServer(botManager: BotManager): APIServerResult {
  ...
  const commanderService = new CommanderService({
    llmClient: botManager.getLLMClient(),
    botManager,
    commandCenter,
    missionManager,
    markerStore,
  ...
  app.post('/api/commander/parse', async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({ err: err?.message }, 'Commander parse endpoint error');
      res.status(500).json({ error: err?.message ?? 'Internal error' });
    }
  });
  ...
  return { app, httpServer, io, eventLog, commandCenter, missionManager, markerStore, squadManager, roleManager, commanderService };
}
