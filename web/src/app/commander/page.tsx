...
export default function CommanderPage() {
  ...
  // Load history, drafts, and suggestions from API on mount (in parallel)
  useEffect(() => {
    let cancelled = false;
    Promise.all([
    ...
    <div className="p-6 lg:p-8 space-y-5 max-w-4xl">
      ...
      {initialLoading && (
        <div className="py-12 text-center">
          ...
          <p className="text-xs text-zinc-500">Loading commander...</p>
        </div>
      )}

      {/* Input area */}
      <div className={`bg-zinc-900/80 border border-zinc-800 rounded-xl p-4 space-y-3${initialLoading ? ' opacity-50 pointer-events-none' : ''}`}>
        <textarea
          ref={textareaRef}
          value={input}
      ...
      {plan && (
        <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-5 space-y-3">
          <h3 className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Parsed Plan</h3>
