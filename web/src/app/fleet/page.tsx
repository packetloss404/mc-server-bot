...
import { api, type RoleAssignmentRecord, type OverrideRecord } from '@/lib/api';
...
export default function FleetPage() {
  ...
  const [roleAssignments, setRoleAssignments] = useState<RoleAssignmentRecord[]>([]);
  ...
  const getRoleForBot = (botName: string) => roleAssignments.find((a) => a.botName === botName) ?? null;
    ...
    <div className="p-6 lg:p-8 max-w-[1400px] space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          {selectedSquad ? (
            <SquadDetail squad={selectedSquad} bots={bots} getRoleForBot={getRoleForBot} overrides={overrides} />
          ) : (
...
function SquadDetail({
  ...
  squad: Squad;
  bots: { name: string }[];
  getRoleForBot: (botName: string) => RoleAssignmentRecord | null;
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        {squad.botNames.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {squad.botNames.map((name) => {
              const role = getRoleForBot(name);
              const roleColor = role ? (ROLE_COLORS[role.role] || '#6B7280') : null;
              const isOverridden = !!overrides[name];
              return (
                <div
                  key={name}
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    {role && roleColor && (
                      ...
                      >
                        <span className="text-xs">{ROLE_ICONS[role.role] || ''}</span>
                        {role.role.replace('-', ' ')}
                      </span>
                    )}
                    {isOverridden && (
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium text-orange-400 bg-orange-500/10 border border-orange-500/25">
                        Override
                      </span>
                    )}
                  </div>
