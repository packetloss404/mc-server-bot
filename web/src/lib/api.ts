...
export const api = {
  ...
  // Roles
  getRoleAssignments: () => fetchJSON<{ assignments: RoleAssignmentRecord[]; overrides?: Record<string, OverrideRecord> }>('/api/roles'),
  getBotOverride: (name: string) => fetchJSON<{ override: OverrideRecord | null }>(`/api/bots/${name}/override`),
  clearBotOverride: (name: string) => fetchJSON<{ success: boolean }>(`/api/bots/${name}/override`, { method: 'DELETE' }),
